...
2. 編輯 `my_cpp_node/src/talker.cpp` 和 `my_cpp_node/src/listener.cpp` 文件：

    === "📄 `my_cpp_node/src/talker.cpp`"
...
### 3-3. Publisher / Subscriber（Python）
...
2. 編輯 `my_py_node/my_py_node/talker.py` 和 `my_py_node/src/listener.py` 文件：

    === "📄 `my_py_node/src/talker.cpp`"
