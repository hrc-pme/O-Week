...
!!! abstract
    ...
    ROS 2 的 Launch 系統提供一套高度模組化的方式，用於同時啟動多個節點、載入參數、命名空間分組與條件式執行等。  
...
!!! abstract
    ...
    將先前的 Publisher-Subscriber、Service-Client、Action 範例整合進一個 Launch 檔，方便一次啟動多個節點。  
...
│   ├── cpp_action_example/
│   ├── py_action_example/
│   └── all_launch_examples/        # 新增的 launch package
│       ├── launch/
...
!!! abstract 
    ...
    本章介紹如何將 ROS 2 專案模組化，分拆多個節點、套件，並進一步支援動態參數與 Lifecycle 管理。  
    這些技巧是建構大型專案的基礎。
